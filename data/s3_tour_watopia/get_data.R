...
post_query <-
  ...
  mutate(leagueRef = paste(tour_name, season, category, sep = "."), racenoref = racenoref)


get_results_list <- function(category, disptypemf) {
  #category <- "JLP"
  #disptypemf <- "F"
  leagueRef <- paste(tour_name, season, category, sep = ".")
  ...
  result_list_all$stage_green_jersey_results %<>% mutate(Time = NA_character_)
...
save(
  ...
  rider_list,
  results_list,
  file = here("data", "s3_tour_watopia", "data.RData")
)
