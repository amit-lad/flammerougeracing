...
stage_profile <- tibble(
  Stage = 1:8,
  Distance = c(56,62,42,43,25,36,93,43),
  Climbing = c(572,483,200,310,472,1603,2209,180),
...
current_position <- race_league |> filter(Team == "ART Racing") |> select(Position) |> sum()
...
ART_rider_table <-
  ...
  left_join(overall_gc, by = c("Rider", "Stages", "Time")) |>
  mutate(
    `Pos (FHRC)` = Position.x,
...
### team position
```{r}
valueBox(current_position, icon = "fa-ranking-star", color = "#101010")
```
...
### m climbed
```{r}
valueBox(format(sum(ART_riders_stage_breakdown$Climbing), big.mark=",", scientific = FALSE), icon = "fa-mountain-sun", color = "#101010")
```
